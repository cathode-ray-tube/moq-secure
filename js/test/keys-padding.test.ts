...
describe("InMemoryKeyStore", () => {
  ...
  it.each([-1, 256, 1.5, NaN, Infinity])(
    ...
    (keyId) => {
      ...
      expect(() => store.setKey(keyId, key)).toThrowError(
        expect.objectContaining({ code: "KeyIdInvalid" }),
      );
    },
  );

  it("rejects keys with the wrong length", () => {
    expect(() => new InMemoryKeyStore().setKey(1, new Uint8Array(31)))
      .toThrowError(expect.objectContaining({ code: "KeyWrongLength" }));
  });
  ...
  it("does not expose missing keys", () => {
    expect(new InMemoryKeyStore().aeadKey(9)).toBeUndefined();
  });
});

describe("padding", () => {
  it("prepends zero bytes", () => {
    expect(prependZeroPadding(
      Uint8Array.from([1, 2, 3]),
      2,
    )).toEqual(Uint8Array.from([0, 0, 1, 2, 3]));
  });

  it("removes padding", () => {
    expect(removeZeroPadding(
  ...
  it.each([-1, 1.1, Number.MAX_SAFE_INTEGER + 1])(
    ...
    (length) => {
      expect(() => prependZeroPadding(new Uint8Array(), length))
        .toThrow(RangeError);
    },
  );

  it("rejects padding longer than plaintext", () => {
    expect(() => removeZeroPadding(new Uint8Array(2), 3))
