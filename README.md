...
- Optionally provide authenticity via **Ed25519 signatures** on **some frames** using the broadcaster's published public key.
...
> Handshake / out-of-band key exchange done separately (such as via separate WebSocket server).
...
|---|---:|---|
| `magic` | 4 bytes | Constant magic value (e.g. ASCII `CPAY`) |
...
## 8) Example (conceptual byte layout)
